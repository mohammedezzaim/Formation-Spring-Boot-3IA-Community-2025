...
public class SpringDataJpaPartie9Application {
	...
	public static void main(String[] args) {
		...
		expensiveProduct.stream().forEach(produit -> {
			System.out.println(produit.getName()+" : "+produit.getPrice()+"DH");
		});


		System.out.println();
		List<Produit> productByYearAndMonth = produitDao.findProductByYearAndMonth("2025-01");
		productByYearAndMonth.stream().forEach(
				produit -> {
					System.out.println(produit.getName()+" : "+produit.getDateCreation());
				}
		);
	}
